...
export default {
    ...
    setup() {
        ...
        const handleSocialSignup = async (providerName: keyof typeof providers) => {
            ...
            const authInstance = authRef.value;
            if (!authInstance) {
        ...
        const handleSignup = async () => {
            ...
            const authInstance = authRef.value as Auth;
            ...
            try {
                await createUserWithEmailAndPassword(authRef.value as Auth, email.value, password.value);
                router.push('/user')
            } catch (err) {
                if (err instanceof Error) {
                    error.value = err.message;
